...
vcpecloud_info = {
    ...
    'reserved_image'     : 'vyatta-vm3.qcow2',
    }
...
class BrocadeVcpeTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # First check for reserved Image and get the corresponding IDs
    ...
    def test_create_delete_vdu_mgmtport(self):
        ...
        logger.debug("BrocadevCPE-CAL-Test: Get VDU response %s", rsp)
        self.assertEqual(rsp.vdu_id, test_vdu_id)

        ### Wait until vdu_state is active
        for i in range(50):
            logger.debug("BrocadevCPE-CAL-Test: Getting VDU iter %d", i)
            rc, rsp = self.cal.get_vdu(self._acct, test_vdu_id)
            self.assertEqual(rc, RwStatus.SUCCESS)
            ...
            if (rsp.state == 'active') and ('management_ip' in rsp) and ('public_ip' in rsp):
                ...
                break
            logger.debug("BrocadevCPE-CAL-Test: Waiting another 2 secs")
            time.sleep(2)
        ...
        self.assertEqual(rc, RwStatus.SUCCESS)
        found = False
        logger.debug("BrocadevCPE-CAL-Test: Get VDU list response %s", rsp)
        for vdu in rsp.vdu_info_list:
            if vdu.vdu_id == test_vdu_id:
               found = True
        self.assertEqual(found, True)
    ...
    def test_create_delete_virtual_link(self):
        ...
        rc = self.cal.delete_virtual_link(self._acct, vlink_id)
        self.assertEqual(rc, RwStatus.SUCCESS)
        logger.info("BrocadevCPE-CAL-Test: Deleted virtual_link with Id: %s" % vlink_id)
    ...
    def test_create_delete_virtual_link_and_vdu(self):
        ...
        """
        logger.info("BrocadevCPE-CAL-Test: Test Create Virtual Link API for VDU")
        vlink_req = self._get_virtual_link_request_info()
    ...
    def test_create_rbsh_vdu(self):
        ...
        """
        logger.info("BrocadevCPE-CAL-Test: Test Create Virtual Link API for RBSH")
        vlink_list = []
        for ctr in range(3):
           vlink = RwcalYang.VirtualLinkReqParams()
