...
class NetconfProxy(Proxy):
    ...
    def populate_keys(xpath, xml, prefix, namespace):
        ...
        for key_path in key_paths:
            ...
            print(" GHI Trying to find keypath in namespaces ", namespaces)
            # Try to find the key
            ...
            if not match:
                ...
                print(" GHI No match 2 ", parent_xpath, namespaces)
                parent_list = tree.xpath(parent_xpath, namespaces=namespaces)
                print(" GHI No match 2b ", parent_list)
                assert(len(parent_list))
                parent = parent_list[0]
                (key_prefix, elem_name) = match.group('key').split(':')
                print(" GHI No match 2c")
                key_element = etree.Element('{'+namespaces[key_prefix]+'}'+elem_name)
                print(" GHI No match 3")
                key_element.text = match.group('value')
                inserts.append((parent, key_element))
                print(" GHI No match 4")
    ...
    def _get_xml(self, xpath):
        ...
        req_obj = Proxy._create_gi_from_xpath(xpath, self.module)
        req_xml = req_obj.to_xml_v2(self.model, req_obj)

        logger.debug("Request GET subtree xml %s", req_xml)
        response = self.session.ncclient_manager.get(('subtree', req_xml))
    ...
    def _edit_config_xml(self, xpath, xml, operation=None, target='candidate'):
        ...
        if operation == 'delete':
            xml = '<xc:config xmlns:xc="urn:ietf:params:xml:ns:netconf:base:1.0">{}</config>'.format(xml)
        else:
            xml = '<config xmlns="urn:ietf:params:xml:ns:netconf:base:1.0">{}</config>'.format(xml)
        ...
        if target == 'candidate':
            logger.debug("Commiting edit config to %s, operation %s", xpath, operation)
            commit_response = self.session.ncclient_manager.commit()
            logger.debug("CommitED edit config to %s", xpath)
            ...
            if 'rpc-error>' in commit_response_xml:
                ...
                raise ProxyRequestError(msg)
            logger.debug("Commit response: %s", pretty_xml(response_xml))
    ...
    def merge_config(self, xpath, obj, target='candidate'):
        desc = obj.retrieve_descriptor()
        print("merge_config: Converting to xml")
        xml = obj.to_xml_v2(self.model)
        print("merge_config: ConvertED to xml")
        xml = NetconfProxy.populate_keys(xpath, xml, desc.xml_prefix(), desc.xml_ns())
        response = self._edit_config_xml(
                xpath=xpath,
                xml=xml,
    ...
    def delete_config(self, xpath, target='candidate'):
        print(" 7891 delete operation on xpath ", xpath)
        stripped_xpath = Proxy._xpath_strip_keys(xpath)
        print(" 7892 delete operation on stripped xpath ", stripped_xpath)
        desc = RwKeyspec.get_pbcmd_from_xpath(stripped_xpath, self.schema)
        print(" 7893 delete operation on desc ", desc)
        print(" 78932 delete operation on typename ", desc.get_gi_typename())
        (module_name, obj_type) = desc.get_gi_typename().split(".", 1)
        print(" 7894 delete operation on xpath ", xpath)
        create_object = getattr(self.module, obj_type)
        obj = create_object()
        print(" 7895 delete operation on xpath ", xpath)
        ...
        print(" 789 match ", match)
        xml = obj.to_xml_v2(self.model)
        print(" 789 xml  ", xml)
        xml = NetconfProxy.populate_keys(xpath, xml, desc.xml_prefix(), desc.xml_ns())
        print(" 789 delete operation on xml ", xml)
        response = self._edit_config_xml(
                xpath=xpath,
                xml=xml,
                operation='delete',
    ...
    def rpc(self, input_obj, rpc_name=None, output_obj=None):
        ...
        if response_xml == "":
            ...
            # Don't use for download-image
            if rpc_name != "download-image":
...
class NetconfSessionImpl(object):
    ...
    def connect(self, timeout):
        ...
        '''
        logger.info("Connecting to confd (%s) SSH port (%s) with (%s:%s)", self.host, self.port,
...
def get_next_rule(src_nat, public_pool):
    iplist = [ rule.translation.address for rule in src_nat.rule ]
    rulelist = [ rule.tagnode for rule in src_nat.rule ]
    if rulelist == []:
       ruletag = 10
    else:
...
def test_nat():
    netconf_session = NetconfSession(host="10.66.0.231", username="vyatta", password="vyatta")
    netconf_session.connect()
    natproxy = netconf_session.proxy(VyattaServiceNatV1Yang)
    xpath = "/vyatta-services-v1:service/nat/destination"
    src_nat = natproxy.get(xpath, list_obj=False)
    print(src_nat)

    ruletag, public_ip = get_next_rule(src_nat, "10.66.26.0/24")
    print("ruletag, public_ip ", ruletag, public_ip)
    vdu_ip = "192.168.1.6"
    ...
    new_src_rule.description = "RIFT Rule"
    new_src_rule.tagnode = ruletag
    new_src_rule.source.address = vdu_ip
    new_src_rule.translation.address = public_ip
    new_src_rule.outbound_interface = "dp0s2f0"
    xpath = "/service/nat/source"
    natproxy.merge_config(xpath, new_src_rule, target='candidate')
    ...
    new_dst_rule.description = "RIFT Rule"
    new_dst_rule.tagnode = ruletag
    new_dst_rule.translation.address = vdu_ip
    new_dst_rule.destination.address = public_ip
    new_dst_rule.inbound_interface = "dp0s2f0"
    xpath = "/service/nat/destination"
    natproxy.merge_config(xpath, new_dst_rule, target='candidate')

    print("Getting config after commits")
    src_nat = natproxy.get(xpath, list_obj=False)
    print(src_nat)

    #print("Deleting NAT rules")
    xpath = "/vyatta-services-v1:service/nat/source/rule[tagnode={}]".format(10)
    natproxy.delete_config(xpath, target='candidate')
    xpath = "/vyatta-services-v1:service/nat/destination/rule[tagnode={}]".format(10)
    natproxy.delete_config(xpath, target='candidate')
    
def get_l3_network_list(dhcpproxy):
        print("BrocadevCPEDriver: Getting L3 network list ")
        ...
        server = dhcpproxy.get(xpath, list_obj=True)
        print("BrocadevCPEDriver: Done getting L3 network list ")
...
def merge_dhcp_server_config(self, server):
        print("BrocadevCPEDriver: Merging DHCP config ")
        xpath = "/vyatta-services-v1:service/dhcp-server"
        print("43 %s %s", xpath, server)
        self.dhcpproxy.merge_config(xpath, server, target='candidate')
        print("BrocadevCPEDriver: Merging DHCP succeeded. ")

def get_dhcp_server_config(dhcpproxy):
        logger.info("TestBrocadevCPEDriver: Getting DHCP config ")
        xpath = "/vyatta-services-v1:service/dhcp-server"
        server = dhcpproxy.get(xpath)
        logger.info("TestBrocadevCPEDriver: Getting DHCP succeeded. ")
        return server
...
def test_dhcp():
    netconf_session = NetconfSession(host="10.66.0.231", username="vyatta", password="vyatta")
    netconf_session.connect()
    dhcpproxy = netconf_session.proxy(VyattaServiceDhcpServerV1Yang)
    network_list = get_l3_network_list(dhcpproxy)
    print(network_list)
    ...
    nw.tagnode = 'mgmt'
    if network_list[0].has_field('disable'):
        print("Got disable for nw %s", nw.tagnode)
        nw.disable = True

    s1 = nw.subnet.add()
    s1.tagnode = "192.168.1.0/24"
    range1 = s1.start.add()
    range1.tagnode = "192.168.1.2"
    range1.stop = "192.168.1.2"
    merge_dhcp_server_config(dhcp_server)
...
def test_dhcp2():
    ...
    netconf_session.connect()
    dhcpproxy = netconf_session.proxy(VyattaServiceDhcpServerV1Yang)
    server = get_dhcp_server_config(dhcpproxy)
    print("Reply: server ", server)

def get_vhost_interfaces(intfproxy):
        xpath = "/vyatta-if-v1:interfaces/vyatta-interfaces-vhost-v1:vhost"
        vhostlist = intfproxy.get(xpath, list_obj=True)
        return vhostlist
   
def test_vhost():
    netconf_session = NetconfSession(host="10.66.0.231", port=22, username="vyatta", password="vyatta")
    netconf_session.connect()
    intfproxy = netconf_session.proxy(VyattaInterfacesVhostPbrV1Yang)
    vhostlist = get_vhost_interfaces(intfproxy)
    print("Reply: vhostlist ", vhostlist)

def get_vm_list(virtproxy):
          xpath = "/virtualization/guest"
          try:
              vmcfglist = virtproxy.get(xpath, list_obj=True)
          except ProxyRequestError as err:
              print("BrocadevCPEDriver: Get VM list failed, Error %s", str(err))
              return None, None
          xpath = "/virtualization/guest-state/guest"
          try:
              vmstatelist = virtproxy.get(xpath, list_obj=True)
          except ProxyRequestError as err:
              print("BrocadevCPEDriver: Get VM state list failed, Error %s", str(err))
              return None, None
...
def test_get_vms():
      netconf_session = NetconfSession(host="10.66.0.231", username="vyatta", password="vyatta")
      netconf_session.connect()
      virtproxy = netconf_session.proxy(VyattaVirtualizationV1Yang)
      vmlist, vmstatelist = get_vm_list(virtproxy)
      print("Reply: vmlist ", vmlist)
      print("Reply: vmstatelist ", vmstatelist)

def get_loop_list(loproxy):
      xpath = "/interfaces/loopback"
      lolist = loproxy.get(xpath, list_obj=True)
      logger.debug("Loopback list %s", lolist)
      return lolist

def test_get_loopbacks():
      netconf_session = NetconfSession(host="10.66.4.26", username="vyatta", password="vyatta")
      netconf_session.connect()
      loproxy = netconf_session.proxy(VyattaInterfacesLoopbackV1Yang)
      lolist = get_loop_list(loproxy)
      print("Reply: loopback list ", lolist)
  
def get_dp_interface(dpproxy, name):
      xpath = "/interfaces/dataplane"
      dplist = dpproxy.get(xpath, list_obj=True)
      logger.debug("Dataplane list %s", dplist)
...
def test_get_dps():
      netconf_session = NetconfSession(host="10.66.0.233", username="vyatta", password="password")
      netconf_session.connect()
      dpproxy = netconf_session.proxy(RiftVyattaCompositeYang)
      dp = get_dp_interface(dpproxy, "dp0s2f1")
      print("Reply: DP  ", dp)
