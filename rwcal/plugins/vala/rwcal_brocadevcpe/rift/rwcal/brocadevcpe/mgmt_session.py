...
class NetconfProxy(Proxy):
    ...
    def get(self, xpath, list_obj=False):
        ...
        if response_xml == "":
            logger.debug("Response contained no data")
            return None
...
class NetconfSessionImpl(object):
    ...
    def connect(self, timeout):
        ...
        '''
        logger.info("Connecting to netconf server (%s) SSH port (%s) with (%s:%s)", self.host, self.port,
        ...
        while True:
            try:
                ...
                logger.info("Successfully connected to netconf server (%s) SSH port (%s)", self.host, self.port)
                self._nc_mgr.timeout = NetconfSessionImpl.OPERATION_TIMEOUT_SECS
                return

            except ncclient.NCClientError as e:
                logger.debug("Could not connect to (%s) netconf server ssh port (%s): %s",
        ...
        raise ProxyConnectionError("Could not connect to netconf server ({}) ssh port ({}): within the timeout {} sec.".format(
    ...
    def close(self):
        ...
        if self._nc_mgr is not None:
            try:
                logger.info("Closing netconf server SSH session to %s", self.host)
                if self._nc_mgr.connected:
                    self._nc_mgr.close_session()
                else:
                    logger.warning("netconf server SSH session to %s already closed.", self.host)
    ...
    def ncclient_manager(self):
        ...
        if self._nc_mgr is None:
            raise ProxyConnectionError("Not connected to netconf server")
