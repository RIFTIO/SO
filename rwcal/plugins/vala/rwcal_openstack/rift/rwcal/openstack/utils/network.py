...
class NetworkUtils(object):
    ...
    def _parse_virtual_cp(self, cp_info):
        ...
        Returns:
          Protobuf GI oject of type RwcalYang.VirtualLinkInfoParams_VirtualConnectionPoints()
        """
        cp = RwcalYang.VirtualLinkInfoParams_VirtualConnectionPoints()
    ...
    def prepare_virtual_link(self, link_params, network_id):
        ...
        if link_params.has_field('virtual_cps'):
            ...
            for vcp in link_params.virtual_cps:
                cp = RwcalYang.VDUInitParams_ConnectionPoints()
                cp.from_dict({k:v for k,v in vcp.as_dict().items()
                              if k in ['name','security_group', 'port_security_enabled', 'static_ip_address']})
