    ...
    def image_update(self, image_id, remove_props = None, **kwargs):
        ...
        try:
            if remove_props is not None:
                glconn.images.update(image_id, remove_props=remove_props)
            else:
                glconn.images.update(image_id, **kwargs)
        except Exception as e:
            logger.error("OpenstackDriver: Update Image operation failed for image_id : %s. Exception: %s" %(image_id, str(e)))
            raise
