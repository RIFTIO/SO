...
def vnfd_proxy(request, mgmt_session):
    return mgmt_session.proxy(VnfdYang)
...
class TestMultipleNsSetup(object):
    def test_onboard_descriptors(self, logger, mgmt_session, descriptors, nsd_proxy, vnfd_proxy):
        ...
        assert len(vnfd_proxy.get_config('/vnfd-catalog').vnfd) == len(vnfds)
        assert len(nsd_proxy.get_config('/nsd-catalog').nsd) == len(nsds)


@pytest.mark.depends('multiple_ns_setup')
@pytest.mark.incremental
class TestMultipleNsInstantiate(object):
    def test_instantiate_ns_mem_check(self, logger, rwvnfr_proxy, nsd_proxy, rwnsr_proxy, cloud_account_name, descriptors):
        """It runs over a loop. In each loop, it instantiates a NS, terminates the NS, checks memory usage of the system.
        During memory check, it verifies whether current system mem usage exceeds base memory-usage by a defined threshold.
        """
        ...
        for idx, seq in enumerate(nsd_instantiate_seq, 1):
            ...
            for nsr in rwnsr_proxy.get('/ns-instance-opdata').nsr:
            ...
            # Terminates the NSR
            rift.auto.descriptor.terminate_nsr(rwvnfr_proxy, rwnsr_proxy, logger)
